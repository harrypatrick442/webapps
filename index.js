...
function createApp(hosts, hostMe){
	...
	//app.use(bodyParser.raw({limit: String(SIZE_LIMIT_MB)+'mb' }));
	...
	{
		app.use(express.static(path.join(frontendFolder, '/scripts')));
...
function serverCreated(app, server, selfHosts, ShutdownManager, hostMe, hosts){
	...
	var mysocketsApp = new Mysockets(appHandler);
	...
	if(hostMe.getOrchestrator()){
		ClientDataOrchestratorServer.initialize(hosts, hostMe.getId());
	}
